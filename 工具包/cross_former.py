...
class CrossFormerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, group_size=7, interval=8, lsda_flag=0,
        ...
        if self.use_cpe:
            self.cpe = nn.Conv2d(in_channels=input_resolution[0], out_channels=input_resolution[0], kernel_size=3,
                                 padding=1, groups=input_resolution[0])
    ...
    def forward(self, x):
        ...
        if self.lsda_flag == 0:
            ...
            x = x.reshape(B, Rh, Rw, I, I, Gh, Gw, C).permute(0, 1, 5, 3, 2, 6, 4,
                                                              7).contiguous()  # B, Rh, Gh, I, Rw, Gw, I, C
        x = x.view(B, Hp, Wp, C)
...
class CrossFormer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=[4], in_chans=3, num_classes=4,
                 embed_dim=96, depths=[2, 2, 2, 2], num_heads=[3, 6, 6, 12],
                 group_size=[7, 7, 7, 7], crs_interval=[8, 4, 2, 1],  mlp_ratio=[4., 4., 4., 4.],
    ...
    def compute_group_size(self, group_size=[7, 7, 7, 7], depths=[2, 2, 2, 2], resolution=[56, 56],
                           group_type='constant'):
        ...
        for i_stage in range(len(depths)):
            ...
            for i_block in range(depths[i_stage]):
                if group_type == 'constant':
                    ...
                    rst_group_size[i_stage].append(
                        group_size[i_stage] if i_stage != 2 or i_block >= 4 else group_size[i_stage] // 2)
                cnt += 1
