...
def main(config):
    ...
    for n_subName in subName:
        ...
        model = ModelSimilarity(
            d_img=128,  # 卷积后送 Transformer 的维度
            d_inner=512, layers=4, n_head=8,
            dropout=0.5, d_out=128,
            feature_aug='mean', feature_compose='cat',
            add_sa=True, num_class=4
        )
        ...
        for epoch in range(1, epochs + 1):
            if config.train:
                ...
                for batch_idx, (flow_batch, ecg_batch, labels) in enumerate(train_dl):
                    ...
                    outputs = model(img1=flow_batch, img2=ecg_batch)
