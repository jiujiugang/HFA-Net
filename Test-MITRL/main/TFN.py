...
class TFN(nn.Module):
    ...
    def __init__(self, input_dims, hidden_dims, out, dropouts, post_fusion_dim):
        ...
        self.audio_subnet = TextSubNet(self.audio_in, 'gru', self.audio_hidden, self.out[0], num_layers=2,
                                       bidirectional=True, dropout=self.audio_prob)
        self.video_subnet = TextSubNet(self.video_in, 'gru', self.video_hidden, self.out[1], num_layers=2,
                                       bidirectional=True, dropout=self.video_prob)
        self.text_subnet = TextSubNet(self.text_in, 'gru', self.text_hidden, self.out[2], num_layers=2,
                                      bidirectional=True, dropout=self.text_prob)

        # define the post_fusion layers
        self.post_fusion_dropout = nn.Dropout(p=self.post_fusion_prob)
        self.post_fusion_layer_1 = nn.Linear((self.out[0] + 1) * (self.out[1] + 1) * (self.out[2] + 1),
