...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape
        #print(f"Before Attention: x.shape={x.shape}, H={H}, W={W}, N={N}")
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)  # 正确的 reshape
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W):
        #print(f"Before Attention: x.shape={x.shape}, H={H}, W={W}")
        attn_out = self.attn(self.norm1(x), H, W)
        #print(f"After Attention: attn_out.shape={attn_out.shape}")

        # 确保形状匹配
        assert x.shape == attn_out.shape, f"Shape mismatch: x={x.shape}, attn_out={attn_out.shape}"
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        H = W = int(x.shape[1] ** 0.5)  # 直接用 N 推导 H, W
        return x, (H, W)
...
class ResT(nn.Module):
    def __init__(self, in_chans=3, num_classes=4, embed_dims=[32, 64, 128, 256],
        ...
        # 修改 PatchEmbed 层
        self.patch_embed_2 = PatchEmbed(patch_size=1, in_ch=embed_dims[0], out_ch=embed_dims[1], with_pos=True)
        self.patch_embed_3 = PatchEmbed(patch_size=1, in_ch=embed_dims[1], out_ch=embed_dims[2], with_pos=True)
        self.patch_embed_4 = PatchEmbed(patch_size=1, in_ch=embed_dims[2], out_ch=embed_dims[3], with_pos=True)
        ...
        self.head = nn.Linear(embed_dims[3] * 4, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x):
        # 1. 经过 Stem 处理
        x = self.stem(x)  # 这里的 x 形状: [B, C, H, W]
        B, _, H, W = x.shape

        # 2. 变换为 Transformer 可用格式 (B, N, C)
        x = x.flatten(2).permute(0, 2, 1)

        # 3. 进入 Stage 1 (浅层 Transformer)
        for blk in self.stage1:
            x = blk(x, H, W)
        x = x.permute(0, 2, 1).reshape(B, -1, H, W)
...
def rest_lite(pretrained=False, **kwargs):
    model = ResT(embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
                 depths=[2, 2, 8, 2], sr_ratios=[8, 4, 2, 1], apply_transform=True, **kwargs)
