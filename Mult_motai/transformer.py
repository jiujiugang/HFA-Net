...
class TransformerEncoder(nn.Module):
    def __init__(self, embed_dim, num_heads, layers, attn_dropout=0.0, relu_dropout=0.0,
        ...
        self.layers = nn.ModuleList([
            TransformerEncoderLayer(embed_dim, num_heads, attn_dropout, relu_dropout,
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None):
        x = self.embed_scale * x_in  # [seq_len, B, D]
        ...
        if x_in_k is not None and x_in_v is not None:
            ...
            if self.embed_positions is not None:
                x_k += self.embed_positions(x_k)
                x_v += self.embed_positions(x_v)
