...
from modules.ImagePatcher import ImagePatcher  # 图像Patch类
#from modules.ImagePatcher import HypParams  # 自己封装一个参数类（见后文）
warnings.filterwarnings("ignore")

class HypParams:
    def __init__(self):
        self.orig_d_l = 768  # ECG patch 序列维度
        self.orig_d_v = 768  # 光流 patch 序列维度
        self.orig_d_a = 768  # 即使你不使用 audio，也要定义，保持兼容
        self.hidden_dim = 64  # 添加这一行

        self.d_l = 64
        self.d_v = 64
        self.d_a = 64  # 同样保持存在即可，不用实际使用
...
def main(config):
    ...
    for n_subName in subName:
        ...
        #model = MULTModel(num_classes=4)
        #model = model.to(device)
        weight_path = os.path.join('ourmodel_CASME3_weights', n_subName + '.pth')
        patcher = ImagePatcher(patch_size=16, in_channels=3).to(device)
        if config.train:
            print('Training ')
        ...
        for epoch in range(1, epochs + 1):
            if config.train:
                ...
                for batch_idx, (flow_batch, ecg_batch, labels) in enumerate(train_dl):
                    ...
                    flow_batch = patcher(flow_batch).to(device)  # [B, N, D]
                    ecg_batch = patcher(ecg_batch).to(device)  # [B, N, D]
                    labels = labels.to(device)#将数据转移到设备上（CPU 或 GPU）

                    optimizer.zero_grad()# 清除之前的梯度
                    outputs, _ = model(x_l=ecg_batch, x_a=None, x_v=flow_batch)
