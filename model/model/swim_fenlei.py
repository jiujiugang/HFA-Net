...
class SwinTransformer_mona(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc = nn.Linear(self.num_features[-1], num_classes)  # 全连接层
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            # 仅保留最后一层的输出（分类任务不需要中间特征）
            if i == self.num_layers - 1:
                x = x_out  # [B, L, C], L = H*W

        # 4. Classification Head
        x = getattr(self, f'norm{self.num_layers - 1}')(x)  # 使用最后一层的归一化
        x = x.mean(dim=1)  # 全局平均池化 [B, C]
        x = self.fc(x)  # 分类预测 [B, num_classes]
        return x
