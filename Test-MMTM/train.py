...
import pandas as pd
import warnings
from model import MMTNet  # 引入 MMTM 模块
...
def main(config):
    ...
    for n_subName in subName:
        ...
        # 初始化模型（直接使用MMTM作为主模型）
        model = MMTNet(num_classes=4)  # 这里直接使用 MMTM 作为模型
        ...
        for epoch in range(1, epochs + 1):
            if config.train:
                ...
                for batch_idx, (flow_batch, ecg_batch, labels) in enumerate(train_dl):
                    ...
                    outputs = model(flow_batch, ecg_batch)  # 直接传递光流和ECG数据到MMTM模型
            ...
            with torch.no_grad():
                for flow_batch, ecg_batch, labels in test_dl:
                    ...
                    outputs = model(flow_batch, ecg_batch)
