...
import pandas as pd
import warnings
from loss_model_dual_msff import DualInception
        ...
        for epoch in range(1, epochs + 1):
            if config.train:
                ...
                for batch_idx, (flow_batch, ecg_batch, labels) in enumerate(train_dl):
                    ...
                    main_output, m4_pooled, t4_pooled = model(x=flow_batch, y=ecg_batch)

                    main_loss = loss_fn(main_output, labels)
                    flow_loss = loss_fn(m4_pooled, labels)
                    ECG_loss = loss_fn(t4_pooled, labels)
                    total_loss = main_loss + flow_loss + ECG_loss
                    ...
                    # 计算训练过程中的准确度等
                    train_loss += total_loss.item() * flow_batch.size(0)
                    num_train_correct += (torch.max(main_output, 1)[1] == labels).sum().item()
            ...
            with torch.no_grad():
                for flow_batch, ecg_batch, labels in test_dl:
                    ...
                    ECG_loss = loss_fn(t4_pooled, labels)
                    total_loss = main_loss + flow_loss + ECG_loss
                    #total_loss = loss_fn(total_loss, labels)

                    val_loss += total_loss.item() * flow_batch.size(0)
                    num_val_correct += (torch.max(main_output, 1)[1] == labels).sum().item()
                    num_val_examples += labels.size(0)
                    all_preds.extend(torch.max(main_output, 1)[1].cpu().numpy())
