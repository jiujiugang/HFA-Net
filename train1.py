...
import torch
from model_dual_msff import DualInception
import numpy as np
...
def recognition_evaluation(final_gt, final_pred, show=False):#定义一个函数 recognition_evaluation，它接受三个参数：final_gt（真实的情感标签），final_pred（模型预测的情感标签），以及一个可选参数 show
    label_dict = {'negative': 0, 'positive': 1, 'surprise': 2,'other': 3}#情感标签映射到数字编码
...
def whole_face_block_coordinates():
    df = pd.read_csv(r'D:\HTNet-master\NEW_MODEL\CASME3_updated1.xlsx')
    base_data_src = r'D:\HTNet-master\NEW_MODEL\CASME_3_APEX'
    ...
    for i in range(len(df)):
        image_name = df['image_name'][i]  # 直接使用image_name列
        img_path_apex = os.path.join(base_data_src, image_name)  # 构建完整路径
...
def crop_optical_flow_block():
    # 其他部分保持不变
    whole_optical_flow_path = r'D:\HTNet-master\NEW_MODEL\ECG_TU_224_224'
...
def main(config):
    ...
    if (config.train):
        if not path.exists('ourmodel_CASME3_weights'):
            os.mkdir('ourmodel_CASME3_weights')#检查是否存在保存模型权重的文件夹，如果不存在则创建。
    ...
    for n_subName in subName:#遍历主数据集下的每个子文件夹
        ...
        weight_path = 'ourmodel_CASME3_weights' + '/' + n_subName + '.pth'

        # Reset or load model weigts
        #kwargs = {'input_channels': 3, 'num_classes': 4}  # 设置你需要的参数
        #model = visformer_tiny(kwargs)  # 将 kwargs 作为参数传递给 create_ConTNet_Ti
        ...
        y_train = torch.Tensor(y_train).to(dtype=torch.long).to(device)#将训练标签转换为长整型Tensor
        four_parts_train =torch.Tensor(np.array(four_parts_train)).permute(0, 3, 1, 2).to(device)#将训练数据从numpy数组转换为Tensor，并调整其维度顺序。
        ...
        four_parts_test = torch.Tensor(np.array(four_parts_test)).permute(0, 3, 1, 2).to(device)
        ...
        for epoch in range(1, epochs + 1):#对每个epoch进行迭代。
            if (config.train):
                ...
                for batch_idx, batch in enumerate(train_dl):  # 添加 batch_idx 以显示进度，#遍历训练数据加载器中的批次
                    ...
                    #if (batch_idx + 1) % accumulation_steps == 0:  # 梯度累加
                    optimizer.step()  # 更新参数
            ...
            if best_accuracy_for_each_subject <= val_acc:#它检查当前的验证准确率是否大于等于之前记录的最佳准确率。
                ...
                if val_acc == 1.0:  # 如果测试准确率为100%
                    directory = 'ourmodel_CASME3_weights'
                    if not os.path.exists(directory):
                        os.makedirs(directory)  # 创建目录
                    weight_path_subject = f'ourmodel_CASME3_weights/{n_subName}_best_accuracy.pth'
